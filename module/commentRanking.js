...
async function getWeeklyNumber() {
  try {
    ...
    let messageTextWeekly = `[info][title]週計ランキング[/title]\n${dayNumber}日目\n`;
...
async function dailyComment() {
  await dailyCommentRanking(CommentMeasurementRoom, sendToRoom);
  const totalMessageNum = await getMessageNum(CommentMeasurementRoom);
  await supabase
    .from("total_message_num")
    .upsert([
      {
        room_id: CommentMeasurementRoom,
        message_num: totalMessageNum,
      }
    ])
  await getMessage(CommentMeasurementRoom);
}
